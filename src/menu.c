...
void init_menus(Menu* unmounted_menu, Menu* mounted_menu) {
    ...
    // Mounted menu setup
    menu_add_item(mounted_menu, "ls-1 (Listar diretório raiz)", app_ls1_callback);
    menu_add_item(mounted_menu, "ls   (Listar todos arquivos e diretórios)", app_ls_callback);
    setup_copy_flow(mounted_menu);
    menu_add_item(mounted_menu, "rm   (Remover arquivo ou diretório)", app_rm_callback);
